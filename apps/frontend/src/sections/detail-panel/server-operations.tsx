...
const ServerOperations: React.FC = () => {
  ...
  if (!context) return <Skeleton className="w-[800px] h-full" />;
  return (
...
const OperationList: React.FC<EditorState> = ({ socket }) => {
        ...
        <Fragment key={i}>
          <div className="flex items-center justify-center">
            <span className="text-sm font-medium text-gray-700 px-2 py-1 rounded">
              revision: <span className="text-blue-600 font-semibold">{i}</span>
            </span>
          </div>
