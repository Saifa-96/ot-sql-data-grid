...
const HighlightedPre = memo(
  ({ children, className, language, ...props }: HighlightedPreProps) => {
    ...
    return (
      <div className="group w-fit overflow-hidden rounded-xl border border-border bg-background shadow-sm">
        <span className="flex w-full items-center justify-between border-b bg-muted p-1 dark:bg-background">
          <Badge
            variant={'secondary'}
          ...
          {/* <CodeCopyButton
            code={code}
            className="text-muted-foreground transition-opacity ease-out group-hover:opacity-100 data-[copied=true]:text-primary data-[copied=false]:opacity-0 data-[copied=true]:opacity-100"
          /> */}
        </span>
        <HighlightedCode
          code={code}
          ...
          className={cn(
            'w-fit overflow-x-auto bg-zinc-950 p-4 text-sm text-zinc-50',
            'dark:!bg-background',
            className,
          )}
          {...props}
