...
class ParserToolKit {
  ...
  protected iterateBetweenParen(cb: (tk: Token) => void) {
    this.expectToken({ type: TokenType.OpenParen });
    while (this.nextIf((tk) => !isEqual({ type: TokenType.CloseParen }, tk))) {
      cb(this.nextToken());
    }
  }
}

export class Parser extends ParserToolKit {
  ...
  private parseCondition() {
    let isNot = this.nextIf((tk) =>
      isEqual({ type: TokenType.Keyword, value: Keyword.Not }, tk)
    );
    let hasOpenParen = this.nextIf((tk) =>
      isEqual({ type: TokenType.OpenParen }, tk)
    );
  }

  private parseComparisonCondition(): ComparisonCondition {
    const isNot = this.nextIf((tk) =>
      isEqual({ type: TokenType.Keyword, value: Keyword.Not }, tk)
    );
  ...
  private parseSelect(): SelectStatement {
    ...
    const tableName = this.parseIdent();
    const tk = this.peekToken();
    if (tk.type === TokenType.Keyword && tk.value === Keyword.Where) {
      console.log("where clause: ", this.parseWhereClause());
    }
